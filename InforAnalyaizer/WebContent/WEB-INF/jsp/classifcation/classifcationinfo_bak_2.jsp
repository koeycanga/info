...
function getJSONclassificationName(tn){
   var res = '{"name":"'+tn.val.classificationName+'","id":"'+tn.val.classification_ID+'","is_show":'+tn.is_show+',"is_open":'+tn.is_open+',"depth":"'+tn.depth+'","i_have_ajax":"'+tn.i_have_ajax+'","children_lg":"'+tn.children_lg+'","imgsrc":"'+tn.imgsrc+'"';
   if(tn.children.length>0){
	   res+=',"children":[';
	   for(var i=0;i<tn.children.length;i++){
		   if(i==0){
...
var ic_tree_dv_checkbox = {
		template: '<div>'+
		           '<div v-if="model.is_show" v-bind:style="t_style">'+
		           '<input type="checkbox"  v-bind:value="model.name"  v-model="checkedNames" class="cy_CMICBMS_checkbox"><label for="cy_CMICBMS_checkbox"></label></div>'+
