...
<body style="background-color: #f2f3f8;">
		...
		<div class="cy_CMICBMS_newfdbd">
			<div class="cy_CMICBMS_newfdtb" v-if="title!='移动文章'"
				...
				<input type="text" class="cy_CMICBMS_newbtinput" placeholder="请输入名称"
					v-model="voForm.collectiontypename">
			</div>
			<!--============点击编辑时树状图出现============-->
				...
				<div class="cy_CMICBMS_newfdip"
					...
					<ul>
						<li class="cy_CIASFE_colletree1st"><a><img
								...
								<div v-on:click="checkTop">我的收藏</div> </a></li>
						<ul>
    ...
    var vm2 = new Vue({         //控制左侧树状菜单的vue对象
        ...
        methods: {
            getTypes: function () {
                ...
                axios.get('getTypes')
                    .then(function (res) {
                        ...
                        vm.voSearch.collectiontypeId = _this.typeVos[0].collectiontypeId;
                        vm.btn_search();
                    })
            ...
            digui: function (typeVos, depth) {              //通过递归为所有节点赋图片值
                var _this = this;
                typeVos.forEach(function (type) {
                    ...
                    if (type.children != []) {
                        depth++;
                        _this.digui(type.children, depth);
                    }
                })
            },
    ...
    var ic_tree_collect = {            //父节点
        ...
        '                            <div v-on:click="btn_checkType(model)">{{model.collectiontypename}}</div> <div class="cy_CIASFE_treefoldered">\n' +
